class Menu < ApplicationRecord
  ...
  def item_counts
    ...
    day_item_counts = SqlQuery.new(:ordered_items_counts, menu_id: id).execute
    ...
    {}.tap do |counts|
      day_item_counts.each do |r|
        ...
        counts[item_id] = {} if counts[item_id].nil?
  ...
  def item_counts_by_pickup_day
    ...
    {}.tap do |counts|
      day_item_counts.each do |r|
        ...
        counts[pickup_day_id] = {} if counts[pickup_day_id].nil?
  ...
  def subscriber_note_html
    @subscriber_note_html ||= MARKDOWN.render(subscriber_note || '').html_safe
  end

  def menu_note_html
    @menu_note_html ||= MARKDOWN.render(menu_note || '').html_safe
  end

  def day_of_note_html
    @day_of_note_html ||= MARKDOWN.render(day_of_note || '').html_safe
  end

  def publish_to_subscribers!
    throw "can only publish_to_subscribers for current week's menu or future week's menu" unless can_publish?
    make_current!
    touch :emailed_at # create audit that email was sent
  ...
  def earliest_deadline
    if pickup_days.loaded?
      ...
      pickup_days.minimum(:order_deadline_at)
    end
  end
  ...
  def latest_deadline
    if pickup_days.loaded?
      ...
      pickup_days.maximum(:order_deadline_at)
    end
  end
  ...
  def ordering_window(deadlines: nil, enforce_week: nil)
    ...
    should_enforce_week = enforce_week.nil? ? !is_special? : enforce_week

    if should_enforce_week
  ...
  def sorted_menu_items(includes: nil)
    ...
    my_menu_items = my_menu_items.includes(includes) if includes.present?
  ...
  def copy_from(original_menu, options = {})
    ...
    sort_offset = menu_items.pluck(:sort_order).compact.max || 0

    original_menu.sorted_menu_items.each_with_index do |original_mi, index|
      new_mi = menu_items.create!(
  ...
  def map_pickup_days_from(original_menu)
    ...
    target_week_start = Time.zone.from_week_id(week_id)

    target_days_by_wday = pickup_days.index_by { |day| day.pickup_at.wday }
  ...
  def copy_notes_from(original_menu, options)
    ...
    attrs[:subscriber_note] = original_menu.subscriber_note if options[:copy_subscriber_note] && subscriber_note.blank?
    attrs[:menu_note] = original_menu.menu_note if options[:copy_menu_note] && menu_note.blank?
    attrs[:day_of_note] = original_menu.day_of_note if options[:copy_day_of_note] && day_of_note.blank?
